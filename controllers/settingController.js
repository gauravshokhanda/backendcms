...
exports.createSetting = async (req, res) => {
    try {
        ...
        const logo = req.file ? `${BASE_URL}/${req.file.path.replace(/\\/g, '/')}` : null;
...
exports.updateSettingById = async (req, res) => {
    try {
        ...
        if (req.file) {
            const logoPath = `${process.env.BASE_URL || "http://localhost:5000"}/uploads/${req.file.filename}`;
        ...
        const updatedSetting = await Setting.findByIdAndUpdate(req.params.id, updateData, {
            new: true,
            runValidators: true,
        });
